...
public class OnMmoAttackMechanic extends Aura implements ITargetedEntitySkill {
    ...
    public OnMmoAttackMechanic(SkillExecutor manager, File file, String skill, MythicLineConfig mlc) {
        ...
        String dSet = mlc.getString(new String[]{"damageset", "set", "s"}, "-1");
        if (!dAdd.equals("0") || !dMult.equals("1") ||  !dSet.equals("-1")) {
            this.modDamage = true;
        }
        ...
        private void exeEvent(AttackEvent event){
            ...
            if (this.executeTargetedAuraSkill(OnMmoAttackMechanic.this.onAttackSkill, meta, target)) {
                ...
                if (OnMmoAttackMechanic.this.modDamage) {
                    PlaceholderContext context = PlaceholderContext.builder().meta(meta).entity(target).build();
                    double add = OnMmoAttackMechanic.this.damageAdd.get(context);
                    double mult = OnMmoAttackMechanic.this.damageMult.get(context);
                    double set = OnMmoAttackMechanic.this.damageSet.get(context);

                    String typeStr = OnMmoAttackMechanic.this.modDamageType == null ? null : OnMmoAttackMechanic.this.modDamageType.get(context);
